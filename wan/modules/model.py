...
class WanSelfAttention(nn.Module):
    """
    WAN DiT Self-Attention + full CharaConsist integration.
    Replaces your old WanSelfAttention entirely.
    """
    ...
    def __init__(
        ...
        fg_share_flag: bool = False,
        bg_share_flag: bool = False,
        t_match: int = 11,
    ):
        super().__init__()
        ...
        self.norm_q = nn.LayerNorm(dim, eps=eps) if qk_norm else nn.Identity()
        self.norm_k = nn.LayerNorm(dim, eps=eps) if qk_norm else nn.Identity()

        # --- CharaConsist parameters ---
        self.text_seq_len   = prompt_token_count      # # of prompt tokens
        self.fg_share_flag  = fg_share_flag
        self.bg_share_flag  = bg_share_flag
        self.t_match        = t_match                 # reference timestep

        # --- state banks ---
        # At each layer we’ll bank:
        #   key, value, attn_out, attn_weights, bg_len, real_len
        self.id_attn_bank: dict[int,dict] = {}
        self.cross_sims = None

    # -------------------- CharaConsist Helpers --------------------

    def get_curr_attn_weights(self, q, k, Hp, Wp):
        bank   = self.id_attn_bank[self.t_match]
        bg_len = bank["bg_len"]
        real   = bank["real_len"]
        # q,k: (B, heads, S, D)
        aw = torch.matmul(
            q[:, :, self.text_seq_len:],          # visual queries
            k[:, :, : real].transpose(-2, -1)     # text+visual keys
        ).to(torch.float16)
        aw = aw.softmax(-1).sum(1)                # sum heads → (B, S_vis)
        aw = rearrange(aw, "b (h w) -> b h w", h=Hp, w=Wp)
        bg = aw[:, :bg_len].mean(1)
        fg = aw[:, bg_len:].mean(1)
        return bg, fg

    def get_curr_cross_sim(self, curr_vis, t):
        id_vis = self.id_attn_bank[t]["attn_out"].to(curr_vis.device)
        return torch.matmul(
            F.normalize(curr_vis, dim=-1),
            F.normalize(id_vis,   dim=-1).transpose(-2, -1)
        )

    def get_expand_attn_mask(self, id_fg_mask, curr_fg_mask, bg_flag, fg_flag, device):
        id_len = id_fg_mask.numel()
        _id = id_fg_mask.view(1,1,1,-1)
        _cr = curr_fg_mask.view(1,1,-1,1)
        expand = _id != _cr
        if not bg_flag:
            expand[:, :, :, ~id_fg_mask] = True
        elif not fg_flag:
            expand[:, :, :,  id_fg_mask] = True
        t2i = torch.ones((1,1,self.text_seq_len,id_len), device=device, dtype=bool)
        return torch.cat((t2i, expand), dim=2)
    ...
    def get_expanded_key_value(
        ...
        bank = self.id_attn_bank[t]
        ori_k = bank["key"].to(device)
        ori_v = bank["value"].to(device)

        saved_k, saved_v = None, None
        curr_mask = curr_fg_mask.flatten().to(device)

        # --- background share ---
        if bg_flag:
            bgm = id_bg_mask.flatten().to(device)
            shared_bg = bgm & (~curr_mask)
            saved_k = ori_k[:, :, shared_bg]
            saved_v = ori_v[:, :, shared_bg]
            
            saved_k = rope_apply(saved_k, grid_sizes, freqs)
            id_fg_mask = torch.zeros(saved_k.shape[2], device=device, dtype=torch.bool)

        # --- foreground share ---
        if fg_flag:
            kfg = ori_k[:, :, id_fg_inds]
            vfg = ori_v[:, :, id_fg_inds]
            kfg = rope_apply(kfg, grid_sizes, freqs)
            if saved_k is not None:
                saved_k = torch.cat([saved_k, kfg], dim=2)
                saved_v = torch.cat([saved_v, vfg], dim=2)
                id_fg_mask = torch.zeros(saved_k.shape[2], device=device, dtype=torch.bool)
                id_fg_mask[-kfg.shape[2]:] = True
            else:
                saved_k, saved_v = kfg, vfg
                id_fg_mask = torch.ones(kfg.shape[2], device=device, dtype=torch.bool)

        # --- build attention mask ---
        expand_mask = self.get_expand_attn_mask(id_fg_mask, curr_mask, bg_flag, fg_flag, device)
        total = self.text_seq_len + self.visual_seq_len + saved_k.shape[2]
        attn_mask = torch.zeros((1,1,self.text_seq_len+self.visual_seq_len,total),
                                device=device, dtype=torch.bfloat16)
        attn_mask[:, :, :, self.text_seq_len+self.visual_seq_len:] = expand_mask

        return saved_k, saved_v, attn_mask

    def ada_tome(self, hidden, t, alpha, id_fg_inds=None, curr_fg_inds=None, max_sim=None):
        bank = self.id_attn_bank[t]
        idhid = bank["attn_out"].to(hidden.device)       # (B, V, C)
        vis   = hidden[:, self.text_seq_len:, :]        # (B, V, C)
        idh   = idhid[:, id_fg_inds]                    # (B, K, C)
        curh  = vis[:, curr_fg_inds]                    # (B, K, C)
        aw    = (alpha * max_sim.flatten()[curr_fg_inds]).view(1,-1,1).to(vis.dtype)
        new   = (1-aw)*curh + aw*idh
        vis[:, curr_fg_inds] = new
        return torch.cat([hidden[:, :self.text_seq_len, :], vis], dim=1)

    def forward(
        self,
        x,                      # (B, L, C)
        seq_lens,               # (B,)
        grid_sizes,             # (B, 3) == (F_p, H_p, W_p)
        freqs,                  # rope freqs
        # ** CharaConsist flags **
        image_rotary_emb=None,
        timestep_ind: int = None,
        ...
        interpolate_weight_dict: dict = {},
        spatial_kwargs: dict = {},
    ):
        B, L, C = x.shape
        Fp, Hp, Wp = grid_sizes[0].tolist()
        self.visual_seq_len = Fp * Hp * Wp

        # --- 1) Q/K/V projections and reshape ---
        q = self.norm_q(self.q(x)).view(B, L, self.num_heads, self.head_dim).transpose(1,2)
        k = self.norm_k(self.k(x)).view(B, L, self.num_heads, self.head_dim).transpose(1,2)
        v =                 self.v(x).view(B, L, self.num_heads, self.head_dim).transpose(1,2)

        # --- 2) Bank K/V at t_match ---
        if save_attn_kv and timestep_ind == self.t_match:
            fg_len = spatial_kwargs.get("id_fg_mask", torch.zeros(0)).sum().item()
            self.id_attn_bank[timestep_ind] = {
                "key":       k.cpu(),
                "value":     v.cpu(),
                "real_len":  self.visual_seq_len,
                "bg_len":    self.visual_seq_len - fg_len
            }
            print(f"[CharaConsist] banked K/V at t={timestep_ind}")

        # --- 3) Save attn_out for sim ---
        if save_attn_out_for_sim and timestep_ind == self.t_match:
            tmp = flash_attention(
            ...
            tmp = tmp.transpose(1,2).reshape(B, L, C)
            self.id_attn_bank[timestep_ind]["attn_out"] = tmp[:,self.text_seq_len:,:].cpu()
            print(f"[CharaConsist] saved attn_out at t={timestep_ind}")

        # --- 4) Save attn weights for mask extraction ---
        if save_attn_weight and timestep_ind == self.t_match:
            bgw, fgw = self.get_curr_attn_weights(q, k, Hp, Wp)
            bank = self.id_attn_bank[timestep_ind]
            bank["attn_weights"] = {"bg":bgw, "fg":fgw}
            print(f"[CharaConsist] saved attn_weights at t={timestep_ind}")

        # --- 6) Inject banked K/V after t_match ---
        attention_mask = None
        if (fg_inter_img_attn or bg_inter_img_attn) and timestep_ind > self.t_match:
            sk, sv, attention_mask = self.get_expanded_key_value(
                grid_sizes,
                freqs,
                bg_inter_img_attn,
                fg_inter_img_attn,
            ...
            sk = rope_apply(sk, grid_sizes, freqs)
            sv = rope_apply(sv, grid_sizes, freqs)
            k = torch.cat([k, sk.to(k.device)], dim=2)
            v = torch.cat([v, sv.to(v.device)], dim=2)
            print(f"[CharaConsist] injected K/V at t={timestep_ind}")

        # --- 7) FlashAttention call ---
        out = flash_attention(
            q=q, k=k, v=v,
            k_lens=seq_lens,
            window_size=self.window_size,
        ...
        # --- 8) Save cross_sim at t_match ---
        if save_cross_sim and timestep_ind == self.t_match:
            self.cross_sims = self.get_curr_cross_sim(out[:,self.text_seq_len:,:], self.t_match)
            print(f"[CharaConsist] saved cross_sims at t={timestep_ind}")

        # --- 9) Adaptive token merging ---
        if attn_out_interpolate and timestep_ind > self.t_match:
            alpha = interpolate_weight_dict.get(self.t_match, 1.0)
            out   = self.ada_tome(out, self.t_match, alpha, **spatial_kwargs)
            print(f"[CharaConsist] applied ada_tome at t={timestep_ind}")
